...
import com.koreait.vehicleservice.kakao.KaKaoService;
import com.koreait.vehicleservice.kakao.KakaoOAuthToken;
import com.koreait.vehicleservice.kakao.KakaoProfile;
...
public class NaverController {
    ...
    public String naverCallback(String code, HttpSession session){
        ...
        //가입자 혹은 비가입자 체크해서 처리
        UserEntity dbnavUser = naverService.navUserFind(String.valueOf(naverProfile.getResponse().getId()));
        if(dbnavUser == null){
            System.out.println("기존회원이 아니기에 자동 회원가입 진행");
            naverService.navJoin(navUser);
            UserEntity dbnavUser2 = naverService.navUserFind(String.valueOf(naverProfile.getResponse().getId()));
